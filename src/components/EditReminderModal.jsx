...
// src_3/components/EditReminderModal.jsx
import React, { useState } from 'react';
import { View, Text, Modal, TouchableOpacity, StyleSheet } from 'react-native';
import Icon from 'react-native-vector-icons/MaterialIcons';
import DateTimePicker from '@react-native-community/datetimepicker';
...
const EditReminderModal = ({
  ...
  const [showDatePicker, setShowDatePicker] = useState(false);
  const [showTimePicker, setShowTimePicker] = useState(false);
  const [place, setPlace] = useState(initialPlace || 'Home');
      ...
      <View style={styles.overlay}>
        <View style={styles.modalContainer}>
          ...
          <View style={styles.row}>
            ...
            <TouchableOpacity style={styles.placeBtn} onPress={() => setPlace('Home')}>
              <Icon
                name={place === 'Home' ? 'radio-button-checked' : 'radio-button-unchecked'}
                size={20}
                color="#000"
              />
              <Text style={{ marginLeft: 6 }}>Home</Text>
            ...
            <TouchableOpacity style={styles.placeBtn} onPress={() => setPlace('Work')}>
              <Icon
                name={place === 'Work' ? 'radio-button-checked' : 'radio-button-unchecked'}
                size={20}
                color="#000"
              />
              <Text style={{ marginLeft: 6 }}>Work</Text>
          ...
          <View style={styles.buttonRow}>
            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: 'red' }]} onPress={() => {
              // optional: handle delete reminder logic
            }}>
              <Text style={{ color: '#fff' }}>Delete</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: '#ccc' }]} onPress={onClose}>
